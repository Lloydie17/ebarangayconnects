...
import { ResidentService, SitioService, HouseholdService, CategoryService, } from '@app/_services';
...
export class ListComponent implements OnInit {
    ...
    ngOnInit() {
        this.residentService.getAll()
            .pipe(first())
            .subscribe(residents => {
                this.residents = residents.filter(resident => !resident.status);
    ...
    deleteResident(residentId: string) {
        const resident = this.residents.find(x => x.residentId === residentId);
        resident.isDeleting = true;
        this.residentService.deleteResident(residentId)
            .pipe(first())
            .subscribe(() => {
                this.residents = this.residents.filter(x => x.id !== residentId) 
            });
    }
